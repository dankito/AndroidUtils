...
import net.dankito.utils.web.IDownloadManager;
...
public class AndroidDownloadManager extends BroadcastReceiver implements IDownloadManager {
  ...
  @Override
  public void downloadUrlAsync(DownloadInfo downloadInfo, IDownloadCompletedCallback callback) {
    try {
      ...
      String destinationFileName = getDestinationFilename(filename, downloadInfo.getUrl());
      ...
      log.error("Could not start Download for " + downloadInfo.getUrl(), e);
    }
  }

  @NonNull
  protected String getDestinationFilename(String filename, String url) {
    return removeReservedCharacters(filename) + "_" +  DOWNLOAD_TIME_DATE_FORMAT.format(new Date()) + ".apk";
  }
  ...
  protected void askShouldDownloadGetCancelled(final long downloadId, CurrentDownload currentDownload) {
    ...
    try { filename = Uri.parse(downloadInfo.getUrl()).getLastPathSegment(); } catch(Exception ignored) { }
  ...
  protected void cancelDownload(long downloadId, DownloadInfo downloadInfo) {
    removeDownloadFromDownloadManager(downloadId);
